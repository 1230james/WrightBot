...
exports.main = function(message,input,database,client) {
    ...
    DiscFunc.sendMessage(message,"You need the **Manage Server** permission to use this command.");
    }
    ...
    let msg2 = "```css\nConfig Info```"
        + "**__Prefix__** - `"+guildPrefix+"config prefix [prefix]`\nThe prefix WrightBot uses in this server. Provide no argument to view the current prefix. Prefixes are __case in-sensitive__."
        + "\n\n**__Vulgar Text__** - `"+guildPrefix+"config vulgar`\nWhether WrightBot uses more vulgar words in randomly-generated phrases. Use command to toggle."
        + "\n\n**__Announcements Channel__** - `"+guildPrefix+"config announcechannel [channel mention]`\nThe channel WrightBot posts announcements from the developer. Provide no argument to view the current channel set."
    DiscFunc.sendMessage(message,msg+"\n\n"+msg2,"Sent config menu");
    break;
    case "prefix": // Changing prefix
    if (args[1] && args[1] != "") {
        database[message.guild.id].prefix = args[1];
        DiscFunc.sendMessage(message,"This server's prefix has been updated in the Court Record.\nThe prefix is now `" + args[1] + "`","Updated prefix to " + args[1]);
    } else {
        DiscFunc.sendMessage(message,"Your Honor, this server's prefix is `" + database[message.guild.id].prefix + "`","Sent prefix");
    }
    break;
    ...
    DiscFunc.sendMessage(message,"Now " + acceptText + " more gruesome cases.\nVulgar words for randomly-generated phrases are now **" + enabledText + "**.","Updated prefix to " + args[1]);
    break;
    ...
    if (channel) { // If a channel was mentioned
        ...
        DiscFunc.sendMessage(message,"This server's announcement channel has been updated in the Court Record.\nI will now post developer announcements in " + channel + ".","Updated announce channel to " + channel.name);
        } else { // If bot cant post in listed channel
        DiscFunc.sendMessage(message,"Your Honor, that channel cannot be the announcement channel because I do not have the **Send Messages** and/or **Read Messages** permission(s) there!","Tried to update announce channel but failed");
        }
    } else { // If no channel was mentioned
        channel = database[message.guild.id].announceChannel || "not set.\nPlease set one by typing `"+guildPrefix+"announcechannel <channel mention>`!";
        DiscFunc.sendMessage(message,"Your Honor, this server's announcement channel is " + channel,"Sent announce channel");
    }
    default: // lol nothing
    }
    return database
